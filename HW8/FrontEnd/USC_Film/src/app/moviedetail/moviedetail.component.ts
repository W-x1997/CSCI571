    ...
    <div class="modal-body">
      <div class="row">
        ...
        <div id="modal_detail" class="col-lg-8">
            ...
            <div id="icons"> 
              <a title="Visit IMDB" *ngIf="modal_imdb_link"  id="modal_imdb" class="icon" target="_blank" href={{modal_imdb_link}} > <i class="fa fa-imdb fa-2x"> </i></a>
              <a title="Visit Instagram" *ngIf="modal_instagram_link" id="modal_instagram" class="icon"  target="_blank" href={{modal_instagram_link}} > <i class="fa fa-instagram fa-2x" > </i></a>
              <a title="Visit FaceBook" *ngIf="modal_facebook_link" id="modal_fb" class="icon"  target="_blank" href={{modal_facebook_link}} > <i class="fa fa-facebook-official fa-2x" > </i></a>
              <a title="Visit Twitter" *ngIf="modal_twitter_link" id="modal_twitter"  class="icon"   target="_blank" href={{modal_twitter_link}}> <i class="fa fa-twitter fa-2x"> </i></a>
...
export class MoviedetailComponent implements OnInit {
  ...
  recommend_carsouel_title:string = "Recommended Movies";
  similar_carsouel_title:string = "Similar Movies";
  ...
  ngOnInit() {
    ...
    if(item.type == 'movie' && item.id == this.id){
    ...
    this.service.getMovieVideo(this.id).subscribe((response: any) => {
    ...
    this.service.getMovieDetails(this.id).subscribe((response: any) => {
      ...
      let date_str  = new String(response.release_date);	
      this.year = date_str.substring(0, 4) ;
     
      let minutes = response.runtime;
      let h = Math.round(minutes / 60);
      let m = minutes % 60;
      this.duration = h + "hrs " + m + "mins";
  ...
  if (localStorage.getItem('history') == null) {
    let newrecord = {"type" : "movie", "id" : this.id, "poster_path" : this.poster_path, "title": this.title};
    ...
    for (let i = 0; i < history_list.length; i++) {
      if (history_list[i].type == 'movie' && history_list[i].id == this.id) {
  ...
  // if (!found) {
  //   history_list.push({"type" : "movie", "id" : this.id, "poster_path" : this.poster_path, "title": this.title});
  // } 
    history_list.push({"type" : "movie", "id" : this.id, "poster_path" : this.poster_path, "title": this.title});
    ...
    this.service.getMovieReviews(this.id).subscribe((response => {
      ...
      //   let date = new String(review["created_at"]);
        
      //   review["year"] = date.substring(0,4);
        

      //   let month_str = date.substring(5,7);
      //   review["month"] = this.getMonth(month_str);

      //   review["day"] = date.substring(8,10);
      //   review["time"] = getTime(date.substring(11,19));
      // }
    }));


    this.service.getRecommendMovies(this.id).subscribe((response => {
      this.recommend_carsouel_data = response;
    }));

    this.service.getSimilarMovies(this.id).subscribe((response => {
      //console.log(response);
  ...
  add_to_list() {
    if(!this.addbtn_clicked) {
      ...
      if (localStorage.getItem('mylist') == null) {
        let cur_item = {"type" : "movie", "id" : this.id, "poster_path" : this.poster_path, "title": this.title};
        let newmylist = [cur_item];
        localStorage.setItem('mylist', JSON.stringify(newmylist));
      } else {
        let mylist = JSON.parse(localStorage.getItem('mylist')!);
        mylist.push({"type" : "movie", "id" : this.id, "poster_path" : this.poster_path, "title": this.title});
     ...
     // console.log(JSON.stringify(JSON.parse(localStorage.getItem('mylist') || '[]')));
      ...
      let mylist = JSON.parse(localStorage.getItem('mylist') || '[]');
      mylist = mylist.filter((item: any)=>(item["type"] == "tv" || (item["type"] == "movie" && item["id"] != this.id)));
      localStorage.setItem('mylist', JSON.stringify(mylist));
